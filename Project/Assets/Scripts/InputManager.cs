...
namespace com.limphus.extraction_shooter
{
    public class InputManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (!pc) pc = GameManager.PlayerController;
        }
        ...
        {
            if (pc.IsRunning) return; // we can't use melee or shoot if we're running
            ...
            if (firearm.InUse()) //and if we're already using our gun, then there's no point in checking the inputs! i.e. we're already shooting/reloading
            {
                pc.CanRun = false; //we'll tell our player controller that it can't run tho
